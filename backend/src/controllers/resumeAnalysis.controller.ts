...
export const analyzeResume = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!user || !user.resumeUrl) throw new apiError(400, "No resume found for analysis");
  if (!user.jobPreferences || user.jobPreferences.length === 0) throw new apiError(400, "Set at least one job preference before analysis");
  ...
  if (!resumeText || resumeText.trim().length < 50) throw new apiError(400, "Could not extract enough text from resume");
  ...
  try {
    ...
    extractedSkills = extractedSkills.filter(skill => {
      ...
      if (knownArtifacts.some(artifact => s.toLowerCase().includes(artifact))) return false;
