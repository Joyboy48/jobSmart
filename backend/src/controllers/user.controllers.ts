...
export const getCurrentUser = asyncHandler(async (req: Request, res: Response) => {
  res.status(200).json(new apiResponse(200, (req as any).user, "user fetched successfully"));
});
...
export const updateProfileImage = asyncHandler(async (req: Request, res: Response) => {
  const profileImageLocalPath = (req as any).file?.path;
  ...
  const oldLocalFilePath = (req as any).user?.profileImage;
...
export const getUserById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await User.findById(id).select(
    "_id name profileImage bio location skills experience education resumeUrl aiInsights"
  );
...
export const updateBio = asyncHandler(async (req: Request, res: Response) => {
  const { bio } = req.body;
  if (typeof bio !== "string" || !bio.trim()) throw new apiError(400, "Invalid bio");
  const user = await User.findByIdAndUpdate(
    (req as any).user._id,
    { $set: { bio } },
...
export const updateLocation = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (typeof location !== "string" || !location.trim()) throw new apiError(400, "Invalid location");
  const user = await User.findByIdAndUpdate(
    (req as any).user._id,
    { $set: { location } },
    { new: true, select: "-password -refreshToken" }
...
export const updateSkills = asyncHandler(async (req: Request, res: Response) => {
  ...
  const userDoc = await User.findById((req as any).user._id);
  if (!userDoc) throw new apiError(404, "User not found");
  const existingSkills = Array.isArray(userDoc.skills) ? userDoc.skills : [];
  const mergedSkills = Array.from(new Set([...existingSkills, ...skills.map((s: string) => s.trim()).filter(Boolean)]));
...
export const addExperience = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!title || !company || !startDate) throw new apiError(400, "Missing required fields");
...
export const updateExperience = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await User.findOneAndUpdate(
    { _id: (req as any).user._id, "experience._id": expId },
    {
      $set: {
  ...
  if (!user) throw new apiError(404, "Experience not found");
...
export const addEducation = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!institution || !degree || !fieldOfStudy || !startDate) throw new apiError(400, "Missing required fields");
...
export const updateEducation = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { institution, degree, fieldOfStudy, startDate, endDate, description } = req.body;
  const user = await User.findOneAndUpdate(
    { _id: (req as any).user._id, "education._id": eduId },
    {
      $set: {
  ...
  if (!user) throw new apiError(404, "Education not found");
...
export const updateResume = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!file) throw new apiError(400, "No resume file uploaded");
  ...
  if (!uploaded) throw new apiError(500, "Failed to upload resume");
...
export const deleteResume = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!user || !user.resumeUrl) throw new apiError(404, "No resume to delete");
...
export const getAiInsights = asyncHandler(async (req: Request, res: Response) => {
  const user = await User.findById((req as any).user._id).select("aiInsights");
  if (!user) throw new apiError(404, "User not found");
  res.status(200).json(new apiResponse(200, user.aiInsights, "AI insights fetched"));
});

// Update job preferences (target roles)
export const updateJobPreference = asyncHandler(async (req: Request, res: Response) => {
