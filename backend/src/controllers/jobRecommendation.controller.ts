...
import { fetchRemotiveJobs, RemotiveJob } from "../utils/remotive.js";
import { apiResponse } from "../utils/apiResponse.js";
import { apiError } from "../utils/apiError.js";
import { asyncHandler } from "../utils/asyncHandler.js";

// Helper: score a job for a user
function scoreJob(job: RemotiveJob, userSkills: string[], jobPreferences: string[], userLocation?: string): number {
  let score = 0;
  // Title match
  if (jobPreferences.some(pref => job.title.toLowerCase().includes(pref.toLowerCase()))) score += 30;
  ...
  // Location match
  if (userLocation && job.candidate_required_location && job.candidate_required_location.toLowerCase().includes(userLocation.toLowerCase())) score += 20;
  // Remote jobs get a bonus if user has no location
  if (!userLocation && job.candidate_required_location.toLowerCase().includes("anywhere")) score += 10;
...
export const getJobRecommendations = asyncHandler(async (req: Request, res: Response) => {
  ...
  let allJobs: RemotiveJob[] = [];
  const seenJobIds = new Set<number>();
  for (const pref of jobPreferences.length ? jobPreferences : [""]) {
    const remotiveQuery: any = {};
    if (pref) remotiveQuery.search = pref;
    if (userLocation) remotiveQuery.location = userLocation;
