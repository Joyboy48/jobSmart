...
// Update QA type for scoring/feedback
interface QA {
  q: string;
  a: string | null;
  score?: number;
...
// Update InterviewSession to include resumeText
interface InterviewSession {
  ...
  role?: string;
  resumeText?: string;
  resumeStatus: "parsed" | "not_found" | "parse_error";
}
...
function extractPureQuestion(text: string): string {
...
export const startInterview = asyncHandler(async (req: Request, res: Response) => {
  ...
  // Parse resume
  let resumeText = "";
  let resumeStatus: "parsed" | "not_found" | "parse_error" = "parsed";
  try {
    if (user.resumeUrl) {
      let resumeUrl = user.resumeUrl;
      if (resumeUrl.startsWith("http://")) resumeUrl = resumeUrl.replace("http://", "https://");
...
export const answerInterview = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!sessionId || !sessions[sessionId]) throw new apiError(404, "Session not found");
...
Respond ONLY with the JSON object, nothing else. Do NOT include any commentary, code fences, or extra text.`;
  ...
  if (report.strengths) report.strengths = report.strengths.slice(0, 3);
  if (report.weaknesses) report.weaknesses = report.weaknesses.slice(0, 3);
  if (report.resumeComparison) report.resumeComparison = report.resumeComparison.slice(0, 3);
